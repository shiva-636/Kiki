...
import { startGame, performAction, publicGameView, privateGameView, canStart } from './server/games.js';
...
function serializeRoom(room, viewerPlayer) {
  ...
  const payload = {
    ...
    chat: room.chat || [],
    voice: {
      states: Object.fromEntries(room.players.filter((p) => p.active).map((p) => [p.id, Boolean(room.voice?.states?.[p.id])])),
      signals: viewerPlayer ? (room.voice?.signals || []).filter((s) => s.to === viewerPlayer.id).slice(-80) : [],
    },
  ...
  try {
    ...
    if (parts.length >= 3 && parts[1] === 'rooms') {
      ...
      if (sub === 'join' && req.method === 'POST') {
        ...
        const result = store.joinRoom(room, name);
