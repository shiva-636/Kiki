...
import { on, escapeHtml, avatarColor } from '../dom.js?v=5.1';
import { prefersReducedMotion } from '../dom.js?v=5.1';
...
export function renderWheelGame(room, game) {
  ...
  ` : '';
  const truthDarePopup = game === 'truthOrDare' && isMeSelected && !g.chosenType ? `
    <div class="action-popup" role="dialog" aria-label="Truth or Dare choice">
      <div class="action-popup-icon">🎡</div>
      <p class="action-popup-title">You’re up!</p>
      <p class="action-popup-text">Choose your challenge. Your choice will be posted to the group chat.</p>
  ...
  const guessWhoScoring = game === 'guessWho' && selectedId ? `
    ${isCoordinator ? `
      <div class="coord-panel">
        <p class="coord-title">${g.resultsRecorded ? 'Round scored ✅' : 'Who guessed correctly?'}</p>
        ${!g.resultsRecorded ? `
          <div class="chip-grid">
          ...
          <button class="btn btn-primary btn-block" data-action="confirm-guess-results" type="button">Confirm results</button>
        ` : ''}
      </div>
    ` : `<p class="waiting-note">${g.resultsRecorded ? 'Scores are in!' : 'The coordinator is recording who guessed right.'}</p>`}
  ` : '';
  ...
  return `
    <div class="game-panel game-wheel">
      ...
      ${resultBanner}
      ${isCoordinator ? `
        <button class="btn btn-primary btn-lg btn-block" data-action="spin" type="button">${selectedId ? cfg.respinLabel : cfg.spinLabel}</button>
      ` : `<p class="waiting-note">Waiting for the coordinator to spin…</p>`}
      ${thinkerPopup}
      ${thinkerAnswers}
      ${truthDarePopup}
      ${selectedId ? `<p class="wheel-chat-hint">${escapeHtml(cfg.discussPrompt)}</p>` : ''}
      ${renderChat(room)}
...
export function mountWheelGame(root, ctx, game) {
  ...
  on(root, '[data-action="confirm-guess-results"]', 'click', async () => {
    ...
    try {
      ...
      showToast('Results recorded. Each correct guess gets +10.');
    } catch (err) {
      setError(err);
    }
    setLoading(false);
