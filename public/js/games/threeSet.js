...
export function renderThreeSet(room) {
  ...
  if (phase === 'collecting-words') {
    const suggestions = g.wordSuggestions || {};
    const submitted = Boolean(suggestions[you?.id]?.submitted);
    const submittedCount = Object.values(suggestions).filter((v) => v?.submitted).length;
    const word = you?.private?.ownSuggestion || '';
    return `
      <div class="game-panel game-threeset">
        <p class="round-pill">Round ${room.round}</p>
        <div class="deal-block">
          <div class="card-stack" aria-hidden="true">🃏</div>
          <h3>Choose a word for Three Set</h3>
          <p>Everyone suggests one unique word. Each accepted word becomes three matching cards.</p>
          ${submitted ? `
            <p class="waiting-note">Your word <strong>${escapeHtml(word)}</strong> is locked in. Waiting for the others…</p>
          ` : `
            <form data-set-word-form class="set-word-form">
              <input class="text-input" data-set-word maxlength="40" autocomplete="off" placeholder="Example: Iron Man" aria-label="Your Three Set word" required />
              <button class="btn btn-primary btn-block" type="submit">Submit word</button>
            </form>
          `}
          <p class="waiting-note">${submittedCount}/${room.players.length} players have submitted.</p>
          <div class="standings-grid">
            ${room.players.map((p) => `<div class="standing-pill ${suggestions[p.id]?.submitted ? 'is-done' : ''}">${escapeHtml(p.name)} · ${suggestions[p.id]?.submitted ? '✅' : '⏳'}</div>`).join('')}
          </div>
        </div>
      </div>
    `;
...
export function mountThreeSet(root, ctx) {
  ...
  on(root, '[data-set-word-form]', 'submit', async (e, form) => {
    e.preventDefault();
    const input = form.querySelector('[data-set-word]');
    ...
    try {
      await sendAction(session.roomCode, session.playerId, session.token, 'submit-set-word', { word });
    } catch (err) {
  ...
  on(root, '[data-action="react-set"]', 'click', async (e, target) => {
    ...
    try {
      const result = await sendAction(session.roomCode, session.playerId, session.token, 'react-set', { alertVersion: alertVersionAtClick });
      if (result?.result?.points !== undefined) {
